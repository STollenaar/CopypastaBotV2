...
func handler(ctx context.Context, sqsEvent events.SQSEvent) error {
	...
	browser = browser.Unmarshal([]byte(sqsObject.Data))
	if err != nil {
		fmt.Println(err)
		return err
	}
	...
	embed := util.DisplayRedditPost(posts[browser.Page+1].ID, true)[0]

	response := discordgo.InteractionResponseData{
		Embeds:     []*discordgo.MessageEmbed{embed},
		Components: getActionRow(browser.Page+1, browser.SubReddit),
	}
